const devicon = angular.module('devicon', ['ngSanitize', 'ngAnimate']);
...
devicon.controller('IconListCtrl', function ($scope, $http, $compile) {
  ...
  const versionStr = `@${$scope.latestReleaseTagging}`;
  const baseUrl = `https://cdn.jsdelivr.net/gh/${gitHubPath}${versionStr}/`;
  ...
  */
  $scope.selectSvg = function (svgVersion, index) {
    $http
      .get(
        `${baseUrl}icons/${$scope.selectedIcon.name}/${$scope.selectedIcon.name}-${svgVersion}.svg`
      )
      .success(function (data) {
        const svgElement = angular.element(data);
        ...
        if (innerSvgElement === null) {
          ...
          const innerSVG = innerSvgElement.innerHTML;
  ...
  $scope.displayTooltip = function (text, copyBtn) {
    const tooltip = copyBtn.parentElement.getElementsByClassName('tooltip')[0];
